...
class PublishPoseState(EventState):
	"""
	Publishes a 2D pose from userdata so that it can be displayed in rviz. Can be used for MoveBase!
	...
	def execute(self, userdata):
		...
		if self._sub.has_msg(self._topic):
			...
			# go to pose of cube
			target_pose = Pose2D(data.position.x - 0.4, data.position.y, 0.0)
