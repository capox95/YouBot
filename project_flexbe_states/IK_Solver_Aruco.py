...
class IKSolverAruco(EventState):
	'''
	Get IK solution for the box pose in terms of joint values.

	--inclination	 float	    Inclination of the end effector wrt the ground, radians (CCW) 		
	--height	 float	    how much millimiters goes down to grasp the cube (with 0 gripper stay just above, with 20 good grasp)
	...
	def __init__(self, inclination, height):
		'''
		Constructor
		'''
		super(IKSolverAruco, self).__init__(outcomes=['found', 'unavailable'], output_keys=['output_ik'], input_keys = ['pose'])
	...
	def on_enter(self, userdata):
		...
		print("yaw value: ", yaw_deg)

		# value for the joint_5 calculated starting from the tf values
		rotation = yaw		

		self.value = self.inverse(userdata.pose.position, self.height, self.inclination, rotation)
	...
	def inverse(self, p, h, inclination_link, rotation_cube):
		...
		#checking the yaw value in order to minimize the rotation of joint_5 needed.
